...
namespace Simcode.PazCheck.CentralServer
{
    public partial class DataAccessServerWorker : Ssz.DataAccessGrpc.ServerBase.DataAccessServerWorkerBase
    {
        ...
        private void On_ServerContextAddedOrRemoved(object? sender, ServerContextAddedOrRemovedEventArgs args)
        {
            ...
            {
                if (new Any(serverContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_ConnectionToMain)).ValueAsBoolean(false))
                {
                    if (!String.IsNullOrEmpty(serverContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_ThisCentralServerAddress)))
                ...
                else
                {
                    ...
                    {
                        if (!String.IsNullOrEmpty(serverContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_Engine_ProcessModelNames)))
                    ...
                    {
                        if (!String.IsNullOrEmpty(serverContext.ContextParams.GetValueOrDefault(@"OperatorSessionId")) &&
                                !String.IsNullOrEmpty(serverContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_Operator_ProcessModelNames)))
                    ...
                    {
                        if (!String.IsNullOrEmpty(serverContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_EngineSessionId)) &&
                                !String.IsNullOrEmpty(serverContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_ControlEngineServerAddress)))
        ...
        {
            var additionalCentralServerAddress = utilityServerContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_ThisCentralServerAddress)!;
        ...
        private void On_EnginesHost_AddedOrRemoved(ServerContext utilityServerContext, bool added)
        {
            ...
            if (added)
            {
                ...
                enginesHostInfo.ProcessModelNames = CsvHelper.ParseCsvLine(@",", utilityServerContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_Engine_ProcessModelNames));
        ...
        {
            string operatorSessionId = utilityServerContext.ContextParams.GetValueOrDefault(@"OperatorSessionId")!;            
            if (added)
            {
                ...
                operatorSession.ProcessModelNames = CsvHelper.ParseCsvLine(@",", utilityServerContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_Operator_ProcessModelNames));
        ...
        {
            string engineSessionId = utilityServerContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_EngineSessionId)!;
            try
            {                
                if (added)
                {
                    foreach (var processModelingSession in _processModelingSessionsCollection.Values)
                    {
                        ...
                        if (engineSession is not null)
                            engineSession.DataAccessProvider.ServerAddress = utilityServerContext.ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_ControlEngineServerAddress)!;
                    }                    
                }                
            }
            catch
        ...
        public class EnginesHostInfo
        {
            ...
            { 
                get
                {
                    ...
                    return new Any(UtilityServerContexts[0].ContextParams.GetValueOrDefault(DataAccessConstants.ParamName_RunningControlEnginesCount)).ValueAsInt32(false);
                }                 
            }
