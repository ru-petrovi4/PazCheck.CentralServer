...
{    
    public partial class DataAccessServerWorker : Ssz.DataAccessGrpc.ServerBase.DataAccessServerWorkerBase
    {
        ...
        {
            var centralServerAddon = _addonsManager.Addons.OfType<PazCheckCentralServerAddon>().Single();
            ...
            foreach (var addonStatus in addonStatuses.AddonStatusesCollection)
            {
                ...
                addonStatus.SourceId = ConfigurationHelper.GetValue(_configuration, PazCheckConstants.ConfigurationKey_Process_SourceId, @"Simcode.PazCheck.CentralServer");
                addonStatus.SourceIdToDisplay = ConfigurationHelper.GetValue(_configuration, PazCheckConstants.ConfigurationKey_Process_SourceIdToDisplay, @"Simcode.PazCheck.CentralServer");
            }
        ...
        private async Task<byte[]> ReadConfigurationPassthroughAsync(ServerContext serverContext, string recipientPath, ReadOnlyMemory<byte> dataToSend)
        {
            ...
            if (dataToSend.Length == 0)
            {
                ...
                foreach (var configurationFile in configurationFiles.ConfigurationFilesCollection)
                {
                    ...
                    configurationFile.SourceId = ConfigurationHelper.GetValue(_configuration, PazCheckConstants.ConfigurationKey_Process_SourceId, @"Simcode.PazCheck.CentralServer");
                    configurationFile.SourceIdToDisplay = ConfigurationHelper.GetValue(_configuration, PazCheckConstants.ConfigurationKey_Process_SourceIdToDisplay, @"Simcode.PazCheck.CentralServer");                    
                }
        ...
        {
            string? operatorSessionId = serverContext.ContextParams.GetValueOrDefault(@"OperatorSessionId");
